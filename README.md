...
The __purifAI__ package is designed for bulk input prediction about what chemical purification method is suggested for a particular chemical. 

The ML model used in the package is  Extreme Gradient Boosting (XGBoost). It is used to prediction what is the best SPE and LCMS purification method for a particular chemical compound. 
...
User can input a csv file of chemical formulars as SMILES, our first function `calculate_descriptors(self, smiles, ipc_avg=False)` will calculate and convert these chemicla formulars into descriptors and features to be put into the prediction model. Then, user can call `RunSPEPrediction(self, smiles)` for SPE methods prediction;and `RunLCMSPrediction(self, smiles)` for LCMS methods prediction. 
